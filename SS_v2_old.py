...
Adds 2 firms to model (this is addition to SS_v1.py)
...
# Parameters for two firms
cbar = [0.0, 0.0]
alpha_c = np.array([0.4, 0.6])
alpha = np.array([0.3, 0.25]) # capital's share of output for each production sector
# initially, we'll drop input output and consumption output matrices - firms produce unique goods and use
# capital that is their output
...
def get_tilde_c(w, r, n, k0, k, bq, p_c, tilde_p_c):
    '''
    Parameters: Factor prices, labor and capital decisions, bequests

    Returns:    Total composite consumption
    '''
    output = (((1+r)*k0) + w*n*e - k + bq - (cbar*p_c).sum())/tilde_p_c
...
def get_c(tilde_c):
    ...
    Returns:    Total composite consumption
    '''
    output = tilde_c + cbar.sum()

    return output

def get_tilde_p(p_c):
    '''
    Parameters: Output prices

    Returns:    price of composite consumption
    '''
    output = np.prod((alpha_c*p_c)**alpha_c)
...
def get_p(r, w, K, L, Y):
    ...
    Returns:    Price of output in each sector
    '''
    output = 1 # price always one with Cobb-Douglas prod
...
rss
    ...
    tild_c = get_tilde_c(w, r, n, k0, k, bq, p_c, tilde_p_c)
    K = get_K(k)
    L = get_L(n)
    Y = get_Y(K, L)    
    r = get_r(Y,K)
