...
b_ss, EulErr_ss, C1_ss, C2_ss, Y1_ss, Y2_ss, K_ss, K1_ss, K2_ss  = s2f.SS(ss_params, b_guess, SS_graphs)
...
print(Y1_ss - C1_ss - delta * (K_ss-((Y2_ss-C2_ss)/delta)))
    ...
    #     K1, K_constr_tpi1 = s2f.get_K(Gamma1)
    #     if K1 <= 0:
    ...
    #         # Run TPI
    #         tpi_params = np.array([S, beta, sigma, A, alpha, delta, K1,
    #                                K_ss, maxiter_TPI, mindist_TPI, xi])
    #         b_path, c_path, K_path, w_path, r_path, EulErr_path = \
    #             s2f.TPI(tpi_params, Kpath_init, Gamma1, Lpath, TPI_graphs)
